...
class LanczosBidiagonalizeVariable(object):
    def __init__(self, cls=None, max_iter=20):
        self.cls = cls or torch.Tensor
        self.max_iter = max_iter
    ...
    def _reorthogonalize(self, matrix, vector, tol=1e-5):
        ...
        norms = matrix.t().matmul(vector)
        tries = 0
        while any(torch.abs(norms.data) > tol) and tries < 10:
            vector = vector - torch.sum(norms * matrix, dim=1)
            vector = vector / torch.norm(vector)
            norms = matrix.t().matmul(vector)
            tries = tries + 1
    ...
    def lanczos_bidiagonalize(self, matmul_closure, matmul_t_closure, initial_vector, num_rows, num_columns):
        ...
        initial_vector = initial_vector / torch.norm(initial_vector)
        assert(len(initial_vector) == num_columns)
        Q = Variable(self.cls(num_columns, num_iters).zero_())
        P = Variable(self.cls(num_rows, num_iters).zero_())
        beta = Variable(self.cls(num_iters).zero_())
        alpha = Variable(self.cls(num_iters).zero_())
        tol = 1e-5
        Q[:, 0] = initial_vector
        for i in range(num_iters):
            if i == 0:
            ...
            if i > 1:
                norms = P[:, :i].t().matmul(P[:, i])
                tries = 0
                while any(torch.abs(norms.data) > tol) and tries < 10:
                    P[:, i] = P[:, i] - torch.sum(norms * P[:, :i], dim=1)
                    P[:, i] = P[:, i] / torch.norm(P[:, i])
                    norms = P[:, :i].t().matmul(P[:, i])
                    tries = tries + 1
