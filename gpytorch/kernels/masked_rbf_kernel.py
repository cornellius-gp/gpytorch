...
class MaskedRBFKernel(Kernel):
    ...
    def __init__(
        ...
        eps=1e-6,
        mask_percentage=100.0,
        **kwargs
    ):
        super(MaskedRBFKernel, self).__init__(
        ...
        if mask_percentage != 100.0:
            masks = []
            for b in range(self.lengthscale.size(0)):
                how_many = int(max(1.0, mask_percentage * self.lengthscale.size(-1) / 100.0))
                mask = torch.cat([torch.ones(how_many), torch.zeros(ard_num_dims - how_many)])
                mask = mask[torch.randperm(ard_num_dims)]
                masks.append(mask)
            self.mask = torch.stack(masks).unsqueeze(-2).float().cuda()
            print("mask", self.mask[:, 0, :].data.cpu().numpy())
        else:
            self.mask = None
