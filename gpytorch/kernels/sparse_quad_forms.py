...
def _scatter_gradients(
    X: Float[Tensor, "... N D"],
    X_grads_unreduced: Float[Tensor, "*batch I I K D"],
    Si: Long[LongTensor, "*batch I I K"],
):
    batch_shape = X_grads_unreduced.shape[:-4]
    N, D = X.shape[-2:]
    I, _, K = X_grads_unreduced.shape[-4:-1]

    # Prepare X_grads_unreduced and Si for a vmap
    X_grads_unreduced = X_grads_unreduced.view(*batch_shape, (I**2) * K, D)  # *batch * KI^2 x D
    Si = Si.reshape(*batch_shape, (I**2) * K)  # *batch x KI^2

    # Function to reduce X_grads_unreduced
    def reduce_grad(X_grads_unreduced_sub: Float[Tensor, "... K*I1*I2"]) -> Float[Tensor, "... N"]:
...
class SparseQuadForm(torch.autograd.Function):
    ...
    def forward(
        ...
        X: Float[Tensor, "... N D"],
        Sv: Float[Tensor, "... I K"],
        Si: Optional[Long[LongTensor, "... I K"]],
        ...
        Sv_ = Sv[..., None, :, None].expand(*batch_shape, I, I, K, 1)
        if Si is None:
            assert K * I == N
        else:
            Si_ = Si[..., None, :, None].expand(*batch_shape, I, I, K, 1)
        ...
        def _sub_forward(
            ...
            Sv1_sub: Float[Tensor, "... K 1"],
            Sv2_sub: Float[Tensor, "... K 1"],
        ) -> Float[Tensor, "..."]:
            K_sub = kernel_forward(X1_sub, X2_sub)
            siT_K_sj = (Sv1_sub * (K_sub @ Sv2_sub))[..., 0].sum(dim=-1)
            return siT_K_sj

        def _sub_forward_with_index(
            Sv1_sub: Float[Tensor, "... K 1"],
            Sv2_sub: Float[Tensor, "... K 1"],
            Si1_sub: Long[LongTensor, "... K 1"],
            Si2_sub: Long[LongTensor, "... K 1"],
        ) -> Float[Tensor, "..."]:
            X1_sub = _vmap_index_select(X_, Si1_sub[..., 0])
            X2_sub = _vmap_index_select(X_, Si2_sub[..., 0])
            return _sub_forward(X1_sub, X2_sub, Sv1_sub, Sv2_sub)

        # Call s_i^T K s_j for all i, j
        if Si is None:
            sub_forward_fn = _sub_forward
            X_ = X_.view(*batch_shape, I, K, D)
            X_ = X_[..., None, :, :].expand(
                *batch_shape, I, I, K, D
        ...
        forward = torch.vmap(
            torch.vmap(sub_forward_fn, in_dims=-3, out_dims=-1), in_dims=-3, out_dims=-1, chunk_size=chunk_size
        )
    ...
    def backward(ctx, V):
        ...
        if any(ctx.needs_input_grad[0:2]):
            ...
            def _sub_backward(
                ...
                Sv1_sub: Float[Tensor, "... K 1"],
                Sv2_sub: Float[Tensor, "... K 1"],
                V_sub: Float[Tensor, "... 1 1"],
            ) -> Tuple[Float[Tensor, "... K 1"], Float[Tensor, "... K D"], Float[Tensor, "... K D"]]:
                vjp_vector = (Sv1_sub @ Sv2_sub.mT).mul_(V_sub)
                K, (X1_grad, X2_grad) = ctx.kernel_forward_and_vjp(X1_sub, X2_sub, vjp_vector)
                Sv1_grad = (K @ Sv2_sub).mul_(V_sub)
                Sv2_grad = (K.mT @ Sv1_sub).mul_(V_sub)
            ...
            if Si is None:
                ...
                backward_args = X_, X_.transpose(-3, -4), Sv_, Sv_.transpose(-3, -4), V[..., None, None]
            else:
                sub_backward_fn = _sub_backward_with_index
                backward_args = Sv_, Sv_.transpose(-3, -4), Si_, Si_.transpose(-3, -4), V[..., None, None]
            backward = torch.vmap(
                torch.vmap(sub_backward_fn, in_dims=-3, out_dims=-3),
                in_dims=-3,
                out_dims=-3,
            ...
            Sv_grad = Sv1_grads.sum(dim=-3).add_(Sv2_grads.sum(dim=-4)).squeeze(-1)
            ...
            if Si is None:
                X_grad = X1_grads.sum(dim=-3).add_(X2_grads.sum(dim=-4))
                X_grad = X_grad.reshape(*batch_shape, N, D)
            else:
                X_grad = torch.add(
                    _scatter_gradients(X, X1_grads, Si_),
                    _scatter_gradients(X, X2_grads, Si_.transpose(-3, -4)),
                )
        else:
            X_grad = None
            Sv_grad = None
