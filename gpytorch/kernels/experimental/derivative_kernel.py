    ...
    class DerivativeKernel(Kernel):
        ...
        def forward(self, x1, x2, diag=False, x1_eq_x2=None, **params):
            ...
            if not diag:
                ...
                K_dx1_x2_func = vmap(lambda _x1: jacrev(kernf)(_x1, x2))
                K_dx1_x2 = K_dx1_x2_func(x1.unsqueeze(-2))
                K_dx1_x2 = K_dx1_x2.squeeze(-2).squeeze(-3).permute(-1, -3, -2).reshape(n1 * d, n2)

                if x1_eq_x2:
                    K_x1_dx2 = K_dx1_x2.transpose(-2, -1)
                else:
                    # Compute K_{x1, dx2} block the same way (then we'll transpose).
                    K_dx2_x1_func = vmap(lambda _x2: jacrev(kernf)(_x2, x1))
                    K_dx2_x1 = K_dx2_x1_func(x2.unsqueeze(-2))
                    K_dx2_x1 = K_dx2_x1.squeeze(-2).squeeze(-3).permute(-1, -3, -2).reshape(n2 * d, n1)
                    K_x1_dx2 = K_dx2_x1.transpose(-2, -1)

                # Compute K_{dx1, dx2} block.
                K_dx1_dx2_func = vmap(vmap(jacrev(jacrev(kernf, argnums=0), argnums=1)))
                x1_expand = x1.unsqueeze(-2).unsqueeze(-2).expand(n1, n2, 1, d)
                x2_expand = x2.unsqueeze(-3).unsqueeze(-2).expand(n1, n2, 1, d)
