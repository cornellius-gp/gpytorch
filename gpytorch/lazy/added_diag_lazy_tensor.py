...
class AddedDiagLazyTensor(SumLazyTensor):
    ...
    def _init_cache(self):
        *batch_shape, n, k = self._piv_chol_self.shape
        ...
        self._precond_lt = LowRankRootAddedDiagLazyTensor(
            LowRankRootLazyTensor(self._piv_chol_self), self._diag_tensor
        )
    ...
    def _probe_vectors_and_norms(self):
        ...
        _eye_probe_vectors = torch.randn(*self.shape[:-1], num_random_probes, dtype=self.dtype, device=self.device)
        eye_probe_vector_norms = torch.norm(_eye_probe_vectors, p=2, dim=-2, keepdim=True)
        eye_probe_vectors = _eye_probe_vectors.div(eye_probe_vector_norms)
        ...
        p_probe_vectors = L @ _eye_probe_vectors[..., : L.size(-1), :] + diag_root @ _eye_probe_vectors
