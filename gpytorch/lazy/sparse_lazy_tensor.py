...
class SparseLazyTensor(LazyTensor):
    def _check_args(self, indices, values, size):

        if indices.shape[:-2] != values.shape[:-2]:
            return (
    ...
    def __init__(self, indices: Tensor, values: Tensor, sparse_size: Tensor):
        ...
        Args:
            ...
            :param values: `b1 x ... x bk x nse` `tensor` containing values of a `b1 x ... x bk`-sized batch
                    of sparse matrices with `sparse_size`.
            :param sparse_size: `tensor` containing shapes of batched sparse matrices.
        ...
        if num_batches > 1:
            ...
            self._tensor = torch.sparse_coo_tensor(
                indices=indices, values=values, size=tensor_size, device=indices.device
            )
        else:
            self._tensor = torch.sparse_coo_tensor(
                indices=indices, values=values, size=sparse_size.to(torch.int64).numpy(), device=indices.device
            )
    ...
    def compute_effective_batch_index(self, *batch_indices):
        shifted_shapes = (*self.batch_shape.shape[:-1], 1)[1:]
    ...
    def _transpose_nonbatch(self):
        ...
        return self.__class__(indices=new_indices, values=self._tensor._values(), sparse_size=self.shape[-2:])
    ...
    def _matmul(self, rhs: Tensor) -> Tensor:
        ...
        if self.ndimension() == 3:
            ...
            return torch.matmul(self._tensor, rhs)
    ...
    def _mul_constant(self, constant):
        ...
        if self.ndimension() > 2:
            ...
            return self.__class__(
                ...
                sparse_size=self._tensor.size()[-2:],
            )
        else:
            return self.__class__(
                indices=self._tensor._indices(),
                values=constant * self._tensor._values(),
                sparse_size=self._tensor.size(),
            )
    ...
    def _expand_batch(self, batch_shape):
        return self.__class__(
            indices=self._tensor.indices().expand(*batch_shape, self._tensor.indices().shape[-2:]),
            values=self._tensor.values().expand(*batch_shape, self._tensor.values().shape[-1]),
            sparse_size=self.size()[-2:],
        )
    ...
    def _sum_batch(self, dim):
        ...
        return sum(
            [
                self.__class__(indices=indices_split, values=values_split, sparse_size=self.size()[-2:])
    ...
    def _permute_batch(self, *dims):
        ...
        return self.__class__(indices=indices, values=values, sparse_size=self.size()[-2:])
