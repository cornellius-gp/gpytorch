...
class InvQuadLazyVariable(LazyVariable):
    def __init__(self, inv_mat, left_mat, right_mat, added_diag=None):
        ...
        if added_diag is None:
            ...
            if right_mat.ndimension() == 3:
                added_diag = (
                    added_diag.unsqueeze(1)
                    .expand(right_mat.size(0), right_mat.size(1))
                )

        super(InvQuadLazyVariable, self).__init__(inv_mat, left_mat, right_mat, added_diag)
    ...
    def inv_quad_log_det(self, inv_quad_rhs=None, log_det=False):
        ...
        inner_mat = self.left_mat.transpose(-1, -2).matmul(self.left_mat / self.added_diag.unsqueeze(-1))
        ...
        if self.ndimension() == 3:
            ...
            if left_mat_inv_quad_rhs is not None:
                left_mat_inv_quad_rhs = left_mat_inv_quad_rhs.unsqueeze(0).repeat(2, 1, 1)

        inv_quad_parts, log_det_parts = inv_quad_log_det(
            solve_mats,
            inv_quad_rhs=left_mat_inv_quad_rhs,
            log_det=log_det,
        )
        ...
        if inv_quad_rhs is not None:
            ...
            inv_quad_res = (inv_quad_rhs.pow(2) / self.added_diag.unsqueeze(-1)).sum(-1).sum(-1) - inv_quad_res

        log_det_res = None
        if log_det:
            if self.ndimension() == 3:
                log_det_res = log_det_parts[:self.size(0)] - log_det_parts[self.size(0):]
            else:
