...
class _HomoskedasticNoiseBase(Module):
    ...
    def forward(self, *params, shape=None):
        ...
        1) non-batched noise, non-batched input, non-MT -> noise_diag shape is `n`
        2) non-batched noise, non-batched input, MT -> noise_diag shape is `nt`
        3) non-batched noise, batched input, non-MT -> noise_diag shape is `b x n` with b' the broadcasted batch shape
        4) non-batched noise, batched input, MT -> noise_diag shape is `b x nt`
        5) batched noise, non-batched input, non-MT -> noise_diag shape is `b x n`
        6) batched noise, non-batched input, MT -> noise_diag shape is `b x nt`
        7) batched noise, batched input, non-MT -> noise_diag shape is `b' x n`
        8) batched noise, batched input, MT -> noise_diag shape is `b' x nt`
        where `n` is the number of evaluation points and `t` is the number of tasks (i.e. `num_tasks` of self.noise).
        So bascially we should always have this be `b' x nt`, with `b'` appropriately broadcast from the noise parameter
        and input batch shapes. `n` and the input batch shape we get either from shape arg or from the params input.
        For this it should be sufficient if we take in a single `shape` arg, where the convention is that shape[:-1]
        is the batch shape of the input, and shape[-1] is `n`.
        """
        if shape is None:
