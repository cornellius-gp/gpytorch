...
class Likelihood(Distribution):
    ...
    In the case of classification, this might be a Bernoulli distribution, where the probability that y=1 is given by the latent function
    ...
    In either case, to implement a (non-Gaussian) likelihood function, GPyTorch requires that two methods be implemented:
        ...
        1. A forward method that computes predictions p(y*|x*) given a distribution over the latent function p(f*|x*). Typically, this solves or
           approximates the integral:

                p(y*|x*) = \int p(y*|f*)p(f*|x*) df*

        2. A log_probability method that computes the log probability \log p(y|f) from a set of samples of f.
    """
    def forward(self, *inputs, **kwargs):
        ...
        With both exact inference and variational inference, the form of p(f|D,x) or p(f|x) should usually be Gaussian. As a result, input should
        usually be a GaussianRandomVariable specified by the mean and (co)variance of p(f|...).
        """
    ...
    def log_probability(self, f, y):
        ...
        For the purposes of our variational inference implementation, y is an n-by-1 label vector,
        and f is an n-by-s matrix of s samples from the variational posterior, q(f|D).
        """
        raise NotImplementedError
