...
class LazyTensorTestCase(RectangularLazyTensorTestCase):
    ...
    def test_cat_rows(self):
        lazy_tensor = self.create_lazy_tensor()
        evaluated = self.evaluate_lazy_tensor(lazy_tensor)
        new_rows = torch.randn(*lazy_tensor.shape[:-2], 1, lazy_tensor.shape[-1])
        new_point = torch.randn(*lazy_tensor.shape[:-2], 1, 1)
        ...
        concat_solve = torch.solve(rhs, concatenated_lt)[0]
