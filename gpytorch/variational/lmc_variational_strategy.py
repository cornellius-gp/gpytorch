...
class LMCVariationalStrategy(_VariationalStrategy):
    ...
    :param int latent_dim: (Default: 0) Which batch dimension corresponds to the latent function batch.
    ...
    def __init__(
        ...
        if not (batch_shape[latent_dim] == num_latents or batch_shape[latent_dim] == 1):
            raise RuntimeError(
                ...
                f"expected the function dim {self.latent_dim} to be {self.num_latents}."
            )
        ...
        # LCM coefficients
        raw_lmc_coefficients = torch.randn(*batch_shape, self.num_tasks).tril_()
        self.register_parameter("raw_lmc_coefficients", torch.nn.Parameter(raw_lmc_coefficients))

    @property
    def lmc_coefficients(self):
        return self.raw_lmc_coefficients.tril()
