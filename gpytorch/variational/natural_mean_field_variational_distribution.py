...
class NaturalMeanFieldVariationalDistribution(_VariationalDistribution):
    r"""
    ...
    def __init__(self, num_inducing_points, batch_shape=torch.Size([]), mean_init_std=1e-3, **kwargs):
        ...
        neg_prec_init = torch.ones(num_inducing_points).mul(-0.5)
        scaled_mean_init = scaled_mean_init.repeat(*batch_shape, 1)
        neg_prec_init = neg_prec_init.repeat(*batch_shape, 1)
        ...
        self.register_parameter(name="natural_mat_diag", parameter=torch.nn.Parameter(neg_prec_init))
    ...
    def forward(self):
        ...
        natural_mat = self.natural_mat_diag.diag_embed(dim1=-1, dim2=-2)
    ...
    def initialize_variational_distribution(self, prior_dist):
        ...
        self.natural_mat_diag.data.copy_(prior_prec.diagonal(dim1=-1, dim2=-2).mul(-0.5))
