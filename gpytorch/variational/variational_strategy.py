...
from ..settings import _linalg_cholesky, trace_mode
...
class VariationalStrategy(_VariationalStrategy):
    ...
    def _cholesky_factor(self, induc_induc_covar):
        L = psd_safe_cholesky(delazify(induc_induc_covar).type(_linalg_cholesky.value()))
    ...
    def forward(self, x, inducing_points, inducing_values, variational_inducing_covar=None, **kwargs):
        ...
        if L.shape != induc_induc_covar.shape:
            ...
            try:
                ...
                pass
            L = self._cholesky_factor(induc_induc_covar)
        interp_term = L.inv_matmul(induc_data_covar.type(_linalg_cholesky.value())).to(full_inputs.dtype)
    ...
    def __call__(self, x, prior=False, **kwargs):
        if not self.updated_strategy.item() and not prior:
            with torch.no_grad():
                ...
                mean_diff = (variational_dist.loc - prior_mean).unsqueeze(-1).type(_linalg_cholesky.value())
                ...
                covar_root = covar_root.type(_linalg_cholesky.value())
