...
class KMeansBlocker(BaseBlocker):
    ...
    def set_test_blocks(self, new_data: torch.tensor) -> List[torch.LongTensor]:
        ...
        block_membership = torch.cdist(new_data, self.centroids).argsort()[:, 0]
        ...
        # convert each block to n-d tensor and return
        test_blocks = [torch.LongTensor(blocking_index) for blocking_index in blocking_indices]
        return test_blocks
