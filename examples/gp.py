...
from ..constants import TORCH_DEVICE, TORCH_DTYPE
from ..transforms import standardize
from .probabilistic_surrogate import ProbabilisticSurrogate
...
class _ExactGPModel(ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, ard_dims, nu, mean, jitter):
        ...
        base_covar_module = MaternKernel(
            float(nu), lengthscale_constraint=Interval(*LENGTHSCALE_BOUNDS, transform=None), ard_num_dims=ard_dims
        )
...
class SingleTaskGP(object):
    ...
    def reset(self):
        ...
        model = _ExactGPModel(None, None, likelihood)
        self.model = model
