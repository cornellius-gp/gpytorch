...
class TestKroneckerProductLazyTensor(unittest.TestCase):
    def test_matmul_vec(self):
        avar = torch.tensor(a.data, requires_grad=True)
        bvar = torch.tensor(b.data, requires_grad=True)
        cvar = torch.tensor(c.data, requires_grad=True)
        ...
        avar_copy = torch.tensor(a.data, requires_grad=True)
        bvar_copy = torch.tensor(b.data, requires_grad=True)
        cvar_copy = torch.tensor(c.data, requires_grad=True)
    ...
    def test_matmul_vec_random_rectangular_nonbatch(self):
        ...
        ax_copy = torch.tensor(ax.data, requires_grad=True)
        bx_copy = bx.clone()
        cx_copy = cx.clone()
    ...
    def test_matmul_batch_mat(self):
        avar = torch.tensor(a.data.repeat(3, 1, 1), requires_grad=True)
        bvar = torch.tensor(b.data.repeat(3, 1, 1), requires_grad=True)
        cvar = torch.tensor(c.data.repeat(3, 1, 1), requires_grad=True)
        mat = torch.tensor(torch.randn(3, 24, 5), requires_grad=True)
        kp_lazy_var = KroneckerProductLazyTensor(NonLazyTensor(avar), NonLazyTensor(bvar), NonLazyTensor(cvar))
        res = kp_lazy_var.matmul(mat)

        avar_copy = torch.tensor(a.data.repeat(3, 1, 1), requires_grad=True)
        bvar_copy = torch.tensor(b.data.repeat(3, 1, 1), requires_grad=True)
        cvar_copy = torch.tensor(c.data.repeat(3, 1, 1), requires_grad=True)
        mat_copy = torch.tensor(mat.data, requires_grad=True)
