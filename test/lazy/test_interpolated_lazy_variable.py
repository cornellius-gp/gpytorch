...
class TestInterpolatedLazyVariable(unittest.TestCase):
    ...
    def test_matmul(self):
        ...
        left_interp_values = Variable(
            torch.Tensor([[1, 2], [0.5, 1], [1, 3]]).repeat(3, 1), requires_grad=True
        )
        ...
        right_interp_values = Variable(
            torch.Tensor([[1, 2], [2, 0.5], [1, 3]]).repeat(3, 1), requires_grad=True
        )
        right_interp_values_copy = Variable(
            right_interp_values.data, requires_grad=True
        )
        ...
        actual = (
            left_matrix.matmul(base_variable_copy).matmul(right_matrix.t()).matmul(
        ...
        self.assertTrue(
            approx_equal(
                left_interp_values.grad.data, left_interp_values_copy.grad.data
            )
        )
    ...
    def test_batch_matmul(self):
        ...
        left_interp_values = Variable(
            torch.Tensor([[1, 2], [0.5, 1], [1, 3]]).repeat(5, 3, 1), requires_grad=True
        )
        left_interp_values_copy = Variable(left_interp_values.data, requires_grad=True)
        right_interp_indices = Variable(
        ...
        right_interp_values = Variable(
            torch.Tensor([[1, 2], [2, 0.5], [1, 3]]).repeat(5, 3, 1), requires_grad=True
        )
        right_interp_values_copy = Variable(
            right_interp_values.data, requires_grad=True
        )
        ...
        base_lazy_variable_mat = (
            base_lazy_variable_mat.transpose(-1, -2).matmul(base_lazy_variable_mat)
        )
        base_variable = Variable(base_lazy_variable_mat, requires_grad=True)
        base_variable_copy = Variable(base_lazy_variable_mat, requires_grad=True)
        base_lazy_variable = NonLazyVariable(base_variable)
        ...
        for i in range(5):
            ...
            left_matrix_comp.scatter_(
                1, left_interp_indices[i], left_interp_values_copy[i]
            )
            right_matrix_comp.scatter_(
                1, right_interp_indices[i], right_interp_values_copy[i]
            )
        ...
        actual = (
            left_matrix.matmul(base_variable_copy).matmul(
    ...
    def test_inv_matmul(self):
        ...
        left_interp_indices = Variable(
            torch.LongTensor([[2, 3], [3, 4], [4, 5]]), requires_grad=True
        )
        left_interp_values = Variable(
            torch.Tensor([[1, 2], [0.5, 1], [1, 3]]), requires_grad=True
        )
        right_interp_indices = Variable(
            torch.LongTensor([[2, 3], [3, 4], [4, 5]]), requires_grad=True
        )
        ...
        self.assertTrue(
            approx_equal(
                base_lazy_variable.grad.data, base_lazy_variable_copy.grad.data
            )
        )
        self.assertTrue(
            approx_equal(
    ...
    def test_matmul_batch(self):
        ...
        left_interp_values = Variable(torch.Tensor([[1, 2], [0.5, 1], [1, 3]])).repeat(
            5, 3, 1
        )
        right_interp_indices = Variable(
        ...
        right_interp_values = Variable(torch.Tensor([[1, 2], [2, 0.5], [1, 3]])).repeat(
            5, 3, 1
        )
        ...
        actual = (
            left_matrix.matmul(base_lazy_variable_mat).matmul(
                right_matrix.transpose(-1, -2)
            ).matmul(
                test_matrix.data
            )
    ...
    def test_getitem_batch(self):
        ...
        self.assertTrue(
            approx_equal(
                interp_lazy_var[1, :1, :2].evaluate().data, actual[1, :1, :2].data
            )
        )
        self.assertTrue(
