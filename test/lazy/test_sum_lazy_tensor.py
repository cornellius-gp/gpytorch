...
class TestSumLazyTensor(unittest.TestCase):
    ...
    def test_add_jitter(self):
        ...
        self.assertLess(torch.max(torch.abs(lazy_var.evaluate().data - (self.t1_eval + self.t2_eval))), 1e-1)

    def test_inv_matmul(self):
        mat = torch.randn(4, 4)
        res = make_sum_lazy_var().inv_matmul(mat)
        self.assertLess(torch.norm(res.data - (self.t1_eval + self.t2_eval).inverse().matmul(mat)), 1e-3)

    def test_getitem(self):
        res = make_sum_lazy_var()[1, 1]
        self.assertLess(torch.norm(res.data - (self.t1_eval + self.t2_eval)[1, 1]), 1e-3)
