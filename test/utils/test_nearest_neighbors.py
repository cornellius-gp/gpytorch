...
class TestNNUtil(unittest.TestCase):
    def _setup(self, k, dim, batch_shape, train_n, test_n, seq_train_n=None):
        # initialize
        nn_util = NNUtil(k=k, dim=dim, batch_shape=batch_shape)
    ...
    def test_setup(self):
        ...
        nn_util = self._setup(k=k, dim=dim, batch_shape=torch.Size([]), train_n=train_n, test_n=test_n)
        self._run(nn_util, dim=dim, batch_shape=nn_util.batch_shape, train_n=new_train_n, test_n=new_test_n)

        self._setup(k=k, dim=dim, batch_shape=torch.Size([2]), train_n=train_n, test_n=test_n)
        self._run(nn_util, dim=dim, batch_shape=nn_util.batch_shape, train_n=new_train_n, test_n=new_test_n)

        self._setup(k=k, dim=dim, batch_shape=torch.Size([2, 5, 10]), train_n=train_n, test_n=test_n, seq_train_n=25)
        self._run(nn_util, dim=dim, batch_shape=nn_util.batch_shape, train_n=new_train_n, test_n=new_test_n, seq_train_n=30)
    ...
    def test_1Dtoycase(self):
        ...
        # reset
        # set new train data
        train_x = torch.tensor([5., 3, 1, 4]).unsqueeze(-1)
        nn_util.set_nn_idx(train_x)

        test_x = torch.tensor([2., 10])
        nn_idx = nn_util.find_nn_idx(test_x)
        nn_idx = torch.sort(nn_idx, dim=-1)[0]
        expected_idx = torch.tensor([[1, 2, 3], [0, 1, 3]], dtype=torch.int64)
        assert torch.equal(nn_idx, expected_idx)

        seq_train_x = torch.tensor([10. ,2, 5, 6, 9]).unsqueeze(-1)
