...
class TestSpectralGPKernel(unittest.TestCase):
    def test_trapezoidal_integration(self, n=2500):
        ...
        for ii in range(30):
            numpy_integral[ii] = np.trapz(
                density.numpy() * np.cos(2.0 * np.pi * tau[ii] * omega.numpy()), omega.numpy(),
            )
    ...
    def test_kernel(self, seed=1, n=500):
        ...
        true_spectral_density = (
            2
            * (2.0 * math.pi * kernel.lengthscale ** 2) ** 0.5
            * torch.exp(-2.0 * math.pi ** 2 * kernel.lengthscale ** 2 * sgp_kernel.omega ** 2)
        )
