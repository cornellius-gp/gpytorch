...
from spectralgp.kernels import SpectralGPKernel, WrappedFKL, GridKronWrapper
...
class WrappedFKLTest(unittest.TestCase):
    def test_forward(self):
        ...
        wrapped_fkl = GridKronWrapper(WrappedFKL(train_x, train_y, shared=False),
                                    grid=grid)

        omg1 = wrapped_fkl.get_omega(0)
        dens1 = dist1.log_prob(omg1)
        wrapped_fkl.set_latent_params(dens1, 0)
        wrapped_cov1 = wrapped_fkl.base_kernel.base_kernels[0](train_x[:, 0], train_x[:, 0])

        omg2 = wrapped_fkl.get_omega(1)
        dens2 = dist2.log_prob(omg2)
        wrapped_fkl.set_latent_params(dens2, 1)
        wrapped_cov2 = wrapped_fkl.base_kernel.base_kernels[1](train_x[:, 1], train_x[:, 1])
