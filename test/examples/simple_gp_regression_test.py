...
def test_posterior_latent_gp_and_likelihood_without_optimization():
    ...
    posterior_latent_distribution = infer.run(prior, train_x, train_y, optimize=False)
...
def test_posterior_latent_gp_and_likelihood_with_optimization():
    ...
    # Compute posterior distribution
    infer = ExactGPInference(likelihood)
    posterior_latent_distribution = infer.run(prior, train_x, train_y, optimize=True)

    # We should learn optimal hyperparmaters
    # bias should be near 0
    bias_value = posterior_latent_distribution.mean_module.bias.data[0]
    assert(math.fabs(bias_value) < .05)

    # log_lengthscale should be near -1.4
    log_lengthscale_value = posterior_latent_distribution.covar_module.log_lengthscale.data.squeeze()[0]
    assert(log_lengthscale_value < -1.1)
    assert(log_lengthscale_value > -1.6)

    # log_noise should be very small
    log_noise_value = likelihood.log_noise.data.squeeze()[0]
