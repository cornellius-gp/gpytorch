...
class TestWassersteinDistance(unittest.TestCase):
    ...
    def test_wasserstein_univariate_gaussians(self):
        ...
        wasserstein_univariate = (mu_q - mu_p) ** 2 + sigma_sq_q + sigma_sq_p - 2 * torch.sqrt(sigma_sq_p * sigma_sq_q)
        self.assertAlmostEqual(wasserstein(q, p, order=2).item(), wasserstein_univariate.item(), delta=1e-5)
