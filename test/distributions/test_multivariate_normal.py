...
class TestMultivariateNormal(unittest.TestCase):
    def test_multivariate_normal_non_lazy(self):
        ...
        self.assertTrue(
            torch.equal(mvn_times2.covariance_matrix, mvn.covariance_matrix * 4)
        )
        mvn_divby2 = mvn / 2
    ...
    def test_multivariate_normal_batch_non_lazy(self):
        ...
        self.assertTrue(
            approx_equal(
                mvn.scale_tril, torch.diag(covmat.diag().sqrt()).repeat(2, 1, 1)
            )
        )
        mvn_plus1 = mvn + 1
        self.assertTrue(torch.equal(mvn_plus1.mean, mvn.mean + 1))
    ...
    def test_multivariate_normal_lazy(self):
        ...
        self.assertTrue(
            torch.equal(
                mvn_plus1.covariance_matrix.evaluate(), mvn.covariance_matrix.evaluate()
            )
        )
        mvn_times2 = mvn * 2
        self.assertTrue(torch.equal(mvn_times2.mean, mvn.mean * 2))
        self.assertTrue(
            torch.equal(
                mvn_times2.covariance_matrix.evaluate(),
                mvn.covariance_matrix.evaluate() * 4,
            )
        )
        mvn_divby2 = mvn / 2
        self.assertTrue(torch.equal(mvn_divby2.mean, mvn.mean / 2))
        self.assertTrue(
            torch.equal(
                mvn_divby2.covariance_matrix.evaluate(),
                mvn.covariance_matrix.evaluate() / 4,
            )
        )
    ...
    def test_multivariate_normal_batch_lazy(self):
        ...
        self.assertTrue(torch.equal(mvn_times2.mean, mvn.mean * 2))
        self.assertTrue(
            torch.equal(mvn_times2.covariance_matrix.evaluate(), mvn.covariance_matrix.evaluate() * 4)
        )
        mvn_divby2 = mvn / 2
        self.assertTrue(torch.equal(mvn_divby2.mean, mvn.mean / 2))
        self.assertTrue(
            torch.equal(mvn_divby2.covariance_matrix.evaluate(), mvn.covariance_matrix.evaluate() / 4)
        )
        # TODO: Add tests for entropy, log_prob, etc. - this an issue b/c it
        # uses using root_decomposition which is not very reliable
        # self.assertTrue(approx_equal(mvn.entropy(), 4.3157 * torch.ones(2)))
