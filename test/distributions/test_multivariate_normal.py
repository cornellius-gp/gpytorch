...
class TestMultivariateNormal(BaseTestCase, unittest.TestCase):
    ...
    def test_multitask_from_batch(self):
        ...
        mmvn = MultivariateNormal(mean, DiagLazyTensor(variance)).to_multitask_from_batch(task_dim=-1)
        ...
        mmvn = MultivariateNormal(mean, DiagLazyTensor(variance)).to_multitask_from_batch(task_dim=0)
        self.assertTrue(isinstance(mmvn, MultitaskMultivariateNormal))
    ...
    def test_multitask_from_repeat(self):
        mean = torch.randn(2, 3)
        variance = torch.randn(2, 3)
        mmvn = MultivariateNormal(mean, DiagLazyTensor(variance)).to_multitask(num_tasks=4)
        self.assertTrue(isinstance(mmvn, MultitaskMultivariateNormal))
