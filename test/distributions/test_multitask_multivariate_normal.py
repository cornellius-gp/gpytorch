...
class TestMultiTaskMultivariateNormal(unittest.TestCase):
    ...
    def test_multitask_multivariate_normal(self, cuda=False):
        ...
        for dtype in (torch.float, torch.double):
            ...
            variance = 1 + torch.arange(4, dtype=dtype, device=device)
            ...
            self.assertAlmostEqual(mtmvn.log_prob(torch.zeros(2, 2).type_as(mean)).item(), -7.3064, places=4)
            logprob = mtmvn.log_prob(torch.zeros(3, 2, 2).type_as(mean))
            logprob_expected = -7.3064 * torch.ones(3).type_as(mean)
    ...
    def test_multitask_multivariate_normal_batch(self, cuda=False):
        device = torch.device("cuda") if cuda else torch.device("cpu")
        for dtype in (torch.float, torch.double):
            mean = torch.tensor([[0, 1], [2, 3]], dtype=dtype, device=device).repeat(2, 1, 1)
            variance = 1 + torch.arange(4, dtype=dtype, device=device)
            ...
            self.assertTrue(torch.equal(mvn_divby2.mean, mtmvn.mean / 2))
            self.assertTrue(torch.equal(mvn_divby2.covariance_matrix, mtmvn.covariance_matrix / 4))
            self.assertTrue(approx_equal(mtmvn.entropy(), 7.2648 * torch.ones(2).type_as(mean)))
            logprob = mtmvn.log_prob(torch.zeros(2, 2, 2).type_as(mean))
            logprob_expected = -7.3064 * torch.ones(2).type_as(mean)
            self.assertTrue(approx_equal(logprob, logprob_expected))
            logprob = mtmvn.log_prob(torch.zeros(3, 2, 2, 2).type_as(mean))
            logprob_expected = -7.3064 * torch.ones(3, 2).type_as(mean)
            self.assertTrue(approx_equal(logprob, logprob_expected))
            conf_lower, conf_upper = mtmvn.confidence_region()
            self.assertTrue(approx_equal(conf_lower, mtmvn.mean - 2 * mtmvn.stddev))
            self.assertTrue(approx_equal(conf_upper, mtmvn.mean + 2 * mtmvn.stddev))
    ...
    def test_multitask_multivariate_normal_multi_batch(self, cuda=False):
        device = torch.device("cuda") if cuda else torch.device("cpu")
        for dtype in (torch.float, torch.double):
            mean = torch.tensor([[0, 1], [2, 3]], dtype=dtype, device=device).repeat(3, 2, 1, 1)
            variance = 1 + torch.arange(4, dtype=dtype, device=device)
            covmat = torch.diag(variance).repeat(3, 2, 1, 1)
            mtmvn = MultitaskMultivariateNormal(mean=mean, covariance_matrix=covmat)
            self.assertTrue(torch.equal(mtmvn.mean, mean))
            self.assertTrue(approx_equal(mtmvn.variance, variance.repeat(3, 2, 1).view(3, 2, 2, 2)))
            self.assertTrue(torch.allclose(mtmvn.scale_tril, covmat.sqrt()))
            mvn_plus1 = mtmvn + 1
            self.assertTrue(torch.equal(mvn_plus1.mean, mtmvn.mean + 1))
            self.assertTrue(torch.equal(mvn_plus1.covariance_matrix, mtmvn.covariance_matrix))
            ...
            self.assertTrue(torch.equal(mvn_divby2.mean, mtmvn.mean / 2))
            self.assertTrue(torch.equal(mvn_divby2.covariance_matrix, mtmvn.covariance_matrix / 4))
            self.assertTrue(approx_equal(mtmvn.entropy(), 7.2648 * torch.ones(3, 2).type_as(mean)))
            logprob = mtmvn.log_prob(torch.zeros(3, 2, 2, 2).type_as(mean))
            logprob_expected = -7.3064 * torch.ones(3, 2).type_as(mean)
            self.assertTrue(approx_equal(logprob, logprob_expected))
            logprob = mtmvn.log_prob(torch.zeros(3, 2, 2, 2).type_as(mean))
            ...
            self.assertTrue(approx_equal(conf_lower, mtmvn.mean - 2 * mtmvn.stddev))
            self.assertTrue(approx_equal(conf_upper, mtmvn.mean + 2 * mtmvn.stddev))
            self.assertTrue(mtmvn.sample().shape == torch.Size([3, 2, 2, 2]))
            self.assertTrue(mtmvn.sample(torch.Size([3])).shape == torch.Size([3, 3, 2, 2, 2]))
            self.assertTrue(mtmvn.sample(torch.Size([3, 4])).shape == torch.Size([3, 4, 3, 2, 2, 2]))

    def test_multitask_multivariate_normal_multi_batch_cuda(self):
        if torch.cuda.is_available():
            with least_used_cuda_device():
                self.test_multitask_multivariate_normal_multi_batch(cuda=True)
