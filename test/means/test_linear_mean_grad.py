...
from gpytorch.means import LinearMeanGrad
from gpytorch.test.base_mean_test_case import BaseMeanTestCase


class TestLinearMeanGrad(BaseMeanTestCase, unittest.TestCase):
    def create_mean(self, input_size=1, batch_shape=torch.Size(), bias=True, **kwargs):
        return LinearMeanGrad(input_size=input_size, batch_shape=batch_shape, bias=bias)
