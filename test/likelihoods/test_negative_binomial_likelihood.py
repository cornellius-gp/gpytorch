...
from gpytorch.likelihoods import _OneDimensionalLikelihood, NegativeBinomialLikelihood
from gpytorch.test.base_likelihood_test_case import BaseLikelihoodTestCase


class TestNegativeBinomialLikelihood(BaseLikelihoodTestCase, unittest.TestCase):
    seed = 1

    def _create_targets(self, batch_shape=torch.Size([])):
        rate = torch.sigmoid(torch.randn(*batch_shape, 5)) * 3.0
        res = torch.poisson(rate).float()
        return res

    def create_likelihood(self, **kwargs):
        return NegativeBinomialLikelihood(**kwargs)
    ...
    def _test_marginal(self, batch_shape):
        # Likelihood uses the default marginal behavior; no need to test sample vs sample.
        pass
