...
def test_backward():
    ...
    c = torch.randn(3, 4)
    actual_a_grad = torch.eye(3, 4).mm((-(a.inverse() / 2).mm(a.inverse() / 2).mm(c)).t()) * 2 * 2
    actual_c_grad = (a.inverse() / 2).t().mm(torch.eye(3, 4)) * 2

    a_var = Variable(a, requires_grad=True)
    c_var = Variable(c, requires_grad=True)
    out_var = a_var.mul(Variable(b))
    out_var = Invmm()(out_var, c_var) 
    out_var = out_var.mul(Variable(torch.eye(3, 4))).sum() * 2
    out_var.backward()
