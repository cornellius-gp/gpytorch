...
def test_cg_with_tridiag():
    ...
    for i in range(5):
        ...
        assert approx_equal(eigs, approx_eigs)
...
def test_batch_cg():
    ...
    actual = torch.cat([torch.potrs(rhs[i], matrix_chol[i]).unsqueeze(0) for i in range(5)])
    assert approx_equal(solves, actual)


def test_batch_cg_with_tridiag():
    batch = 5
    size = 10
    matrix = torch.DoubleTensor(batch, size, size).normal_()
    matrix = matrix.matmul(matrix.transpose(-1, -2))
    ...
    for i in range(5):
        ...
        for j in range(8):
            ...
            assert torch.mean(torch.abs((eigs - approx_eigs) / eigs)) < 0.05
